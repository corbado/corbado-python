...
from generated.api import UserApi
...
class TestAuthMethod(unittest.TestCase):
    ...
    def testAuthMethod(self):
        ...
        create_request = UserCreateReq(name="FullNameCreate",fullName="CreateRequestTestName", email="test3@python.com")
        print("Creating existing client")
        try:
            create_response: UserCreateRsp = userApi.user_create(user_create_req=create_request)
            print(f'Create response: {create_response}')
        except BadRequestException as e:
            print("Exception type:", type(e))
            data = json.loads(e.body)

            print("Exception message:", data.get('error',{}).get('validation',[])[0].get('message' ,{}))
