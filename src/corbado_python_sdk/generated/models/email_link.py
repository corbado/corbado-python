...
class EmailLink(BaseModel):
    ...
    EmailLink
    """ # noqa: E501
    id: StrictStr = Field(description="ID of the email magic link", alias="ID")
    ...
    status: StrictStr = Field(description="status values of an email link")
    additional_payload: StrictStr = Field(description="Additional payload in JSON format", alias="additionalPayload")
    __properties: ClassVar[List[str]] = ["ID", "userID", "email", "userFullName", "purpose", "created", "updated", "status", "additionalPayload"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['active', 'confirmed']):
            raise ValueError("must be one of enum values ('active', 'confirmed')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of EmailLink from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of EmailLink from a dict"""
        if obj is None:
            return None
