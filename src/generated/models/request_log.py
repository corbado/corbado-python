...
class RequestLog(BaseModel):
    ...
    application: StrictStr = Field(description="Application this request was processed with")
    method: StrictStr = Field(description="HTTP method (such as GET and POST)")
    endpoint: StrictStr = Field(description="Endpoint that was requested")
    source: StrictStr = Field(description="Request source")
    request: StrictStr = Field(description="Request JSON data")
    request_headers: Dict[str, StrictStr] = Field(description="Request headers", alias="requestHeaders")
    var_query_params: StrictStr = Field(description="Request query parameters", alias="queryParams")
    response_status: Union[StrictFloat, StrictInt] = Field(description="Response HTTP status", alias="responseStatus")
    response: StrictStr = Field(description="Response JSON data")
    runtime: Union[StrictFloat, StrictInt] = Field(description="Runtime in seconds for this request")
    remote_address: StrictStr = Field(description="Caller remote address", alias="remoteAddress")
    created: StrictStr = Field(description="Timestamp of when the request was performed in RFC3339 format")
    tags: Dict[str, Any] = Field(description="Arbitrary tags attached to this request")
    details: List[StrictStr] = Field(description="Any freetext additional information attached to this request. Additional logs, errors, etc.")
    __properties: ClassVar[List[str]] = ["requestID", "projectID", "userID", "application", "method", "endpoint", "source", "request", "requestHeaders", "queryParams", "responseStatus", "response", "runtime", "remoteAddress", "created", "tags", "details"]
