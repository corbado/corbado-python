...
class WebhookLog(BaseModel):
    ...
    Webhook log entry
    """ # noqa: E501
    webhook_id: StrictStr = Field(description="Unique ID of webhook request which will be randomly generated on server side", alias="webhookID")
    project_id: StrictStr = Field(description="ID of project", alias="projectID")
    action: StrictStr = Field(description="Webhook action")
    response_id: StrictStr = Field(description="Unique ID of response, you can provide your own while responding to the webhook, if not the ID will be randomly generated on server side. This has the same meaning as overriding requestID for API requests. ", alias="responseID")
    request_url: StrictStr = Field(description="Absolute request URL of webhook backend", alias="requestURL")
    request_body: StrictStr = Field(description="Request contents that were sent to webhook backend", alias="requestBody")
    response_status: StrictInt = Field(description="Response HTTP status that was returned by webhook backend", alias="responseStatus")
    response_headers: Dict[str, Any] = Field(description="Response HTTP headers that were returned by webhook backend", alias="responseHeaders")
    response_body: StrictStr = Field(description="Response body JSON data that was returned by webhook backend", alias="responseBody")
    response_error: StrictStr = Field(description="Response error if present", alias="responseError")
    runtime: Union[StrictFloat, StrictInt] = Field(description="Runtime in seconds for this request")
    created: StrictStr = Field(description="Timestamp of when the request was performed in RFC3339 format")
    __properties: ClassVar[List[str]] = ["webhookID", "projectID", "action", "responseID", "requestURL", "requestBody", "responseStatus", "responseHeaders", "responseBody", "responseError", "runtime", "created"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "responseError": obj.get("responseError"),
