...
class WebAuthnAuthenticateFinishRsp(BaseModel):
    """
    ...
    username: StrictStr = Field(description="Username of current challenge")
    credential_id: StrictStr = Field(description="Used credential ID", alias="credentialID")
    ...
    status: StrictStr = Field(description="Status represents information if user signup was successful or the user with its credentials already exists")
    __properties: ClassVar[List[str]] = ["httpStatusCode", "message", "requestData", "runtime", "userID", "username", "credentialID", "userFullName", "status"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['success', 'unconfirmedCredential']):
            raise ValueError("must be one of enum values ('success', 'unconfirmedCredential')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of WebAuthnAuthenticateFinishRsp from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of WebAuthnAuthenticateFinishRsp from a dict"""
        if obj is None:
            return None
