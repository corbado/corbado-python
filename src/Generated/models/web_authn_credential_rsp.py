...
class WebAuthnCredentialRsp(BaseModel):
    """
    ...
    __properties: ClassVar[List[str]] = ["httpStatusCode", "message", "requestData", "runtime", "status"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['pending', 'active', 'deleted']):
            raise ValueError("must be one of enum values ('pending', 'active', 'deleted')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of WebAuthnCredentialRsp from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of WebAuthnCredentialRsp from a dict"""
        if obj is None:
            return None
