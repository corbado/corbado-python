...
class EmailLinkSendReq(BaseModel):
    """
    EmailLinkSendReq
    """ # noqa: E501
    email: StrictStr = Field(description="Recipient email address")
    create: StrictBool = Field(description="Defines if user email should be created if not found")
    token_lifetime: Optional[StrictStr] = Field(default=None, description="Defines the lifetime of the token that needs to be validated", alias="tokenLifetime")
    user_full_name: Optional[StrictStr] = Field(default=None, description="Optional user's full name to be used if the user wasn't found and needs to be created first", alias="userFullName")
    template_name: Optional[StrictStr] = Field(default=None, description="Template name of email to send", alias="templateName")
    purpose: Optional[StrictStr] = Field(default='authentication', description="Purpose of the email link")
    redirect: StrictStr = Field(description="Redirect target after user clicks on email magic link")
    additional_payload: Optional[StrictStr] = Field(default=None, description="Additional payload in JSON format", alias="additionalPayload")
    ...
    __properties: ClassVar[List[str]] = ["email", "create", "tokenLifetime", "userFullName", "templateName", "purpose", "redirect", "additionalPayload", "requestID", "clientInfo"]
    ...
    def purpose_validate_enum(cls, value):
        ...
        if value not in set(['authentication', 'confirmation', 'invitation']):
            raise ValueError("must be one of enum values ('authentication', 'confirmation', 'invitation')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of EmailLinkSendReq from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of EmailLinkSendReq from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "tokenLifetime": obj.get("tokenLifetime"),
            "userFullName": obj.get("userFullName"),
            "templateName": obj.get("templateName"),
            "purpose": obj.get("purpose") if obj.get("purpose") is not None else 'authentication',
            "redirect": obj.get("redirect"),
            "additionalPayload": obj.get("additionalPayload"),
