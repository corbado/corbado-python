...
class PhoneNumberValidationResult(BaseModel):
    """
    PhoneNumberValidationResult
    """ # noqa: E501
    is_valid: StrictBool = Field(alias="isValid")
    validation_code: StrictStr = Field(alias="validationCode")
    phone_number: Optional[ValidationPhoneNumber] = Field(default=None, alias="phoneNumber")
    __properties: ClassVar[List[str]] = ["isValid", "validationCode", "phoneNumber"]

    @field_validator('validation_code')
    def validation_code_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['valid', 'invalid_country_code', 'invalid_number', 'too_long']):
            raise ValueError("must be one of enum values ('valid', 'invalid_country_code', 'invalid_number', 'too_long')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PhoneNumberValidationResult from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PhoneNumberValidationResult from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "phoneNumber": ValidationPhoneNumber.from_dict(obj["phoneNumber"]) if obj.get("phoneNumber") is not None else None
        })
        return _obj
