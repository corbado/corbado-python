...
class WebAuthnAuthenticateStartRsp(BaseModel):
    """
    ...
    public_key_credential_request_options: StrictStr = Field(description="Contains JSON payload data to start Passkeys (Biometrics) login challenge", alias="publicKeyCredentialRequestOptions")
    status: StrictStr = Field(description="Status represents information if authenticate start was successful or device is unknown")
    __properties: ClassVar[List[str]] = ["httpStatusCode", "message", "requestData", "runtime", "publicKeyCredentialRequestOptions", "status"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['success', 'unknownDevice', 'unconfirmedDevice']):
            raise ValueError("must be one of enum values ('success', 'unknownDevice', 'unconfirmedDevice')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of WebAuthnAuthenticateStartRsp from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of WebAuthnAuthenticateStartRsp from a dict"""
        if obj is None:
            return None
