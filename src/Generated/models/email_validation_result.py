...
class EmailValidationResult(BaseModel):
    ...
    email: Optional[ValidationEmail] = None
    __properties: ClassVar[List[str]] = ["isValid", "validationCode", "suggestion", "email"]

    @field_validator('validation_code')
    def validation_code_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['valid', 'invalid_syntax', 'no_such_host', 'not_allowed', 'unknown']):
            raise ValueError("must be one of enum values ('valid', 'invalid_syntax', 'no_such_host', 'not_allowed', 'unknown')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "email": ValidationEmail.from_dict(obj["email"]) if obj.get("email") is not None else None
        })
        return _obj
