...
class WebAuthnRegisterFinishRsp(BaseModel):
    """
    ...
    status: StrictStr = Field(description="Status represents information if user sign up was successful or if the user with provided credentials already exists")
    ...
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['success', 'duplicate']):
            raise ValueError("must be one of enum values ('success', 'duplicate')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of WebAuthnRegisterFinishRsp from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of WebAuthnRegisterFinishRsp from a dict"""
        if obj is None:
            return None
